...
prompt_rsegment() {
  ...
  [[ -n $2 ]] && fg="%F{$2}" || fg="%f"
  if [[ $CURRENT_BG != 'NONE' && $1 != $CURRENT_BG ]]; then
    print -n " %{%k%F{$1}%}%{%k%K{$CURRENT_BG}%}$RSEGMENT_SEPARATOR%{$bg%}%{$fg%} "
  else
    print -n "%{%k%F{$1}%}$RSEGMENT_SEPARATOR%{$bg%}%{$fg%} "
  fi
  CURRENT_BG=$1
  [[ -n $3 ]] && print -n $3
}
...
prompt_time() {
  prompt_rsegment blue white " %{$fg_bold[white]%}%D{%a %e %b - %H:%M}%{$fg_no_bold[white]%} "
}
